...
const fnList = NavObj.map((data,index)=>{
  return <li key={data.key}><a href={data.href}>{data.content}</a></li>
})
// for, forEach 로 사용하던 반복구분으로 쓰기보다,
// 요소의 개념으로 처리할 경우에는 map, filter 의 개념을 쓰는것이 더 효율적이다.
// 컴포넌트 개념으로 적용되는 파일중 js문서에서는 함수와, 이벤트핸들러, 다양한 기능들이 혼합되어있기 때문에 
