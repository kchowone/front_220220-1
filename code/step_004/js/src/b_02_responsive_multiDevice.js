...
const tableCheck = window.matchMedia(`(min-width:${deviceSize[0]}px) and (max-width:${deviceSize[1] - 1}px)`);
...
const deviceCheck = [phoneCheck,tableCheck,laptopCheck,desktopCheck];

deviceCheck.forEach((type,index)=>{
  type.addEventListener('change',function(e){
    if(e.matches){
      console.log('변화1!!!!!', index);
    }
  })
});
