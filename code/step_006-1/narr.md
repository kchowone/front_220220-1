...
2. 문법 jsx는 js문서 내부에서 html코드를 그대로 사용할 수 있도록 만들어진 형태

3. html형태인 jsx내부에서 js코드를 사용하기 위해서는 {}를 사용하여 처리

4. 단, style의 경우에는 {}내부에 {}를 한번 더 사용하여야 하며, 문법은 js와 동일하게 camelCase방식으로 사용
(ex: style = {{fontFamily: 'sans-serif', backgroundImage:'url()'}} )

5. component는 일종의 함수로 이해하며, function Box(){}는 box라는 함수이자 Box 컴포넌트

6. 실제로 jsx 기법으로 사용시에는 `<Box /> `|| `<Box></Box>` 형태로 사용한다.

7. 가장 상위의 위치에서는 (컴포넌트포함) 하나의 요소 또는 하나의 컴포넌트만 존재
여러개가 동일한 레벨일 경우 하나의 빈 컴포넌트를 감싸서 사용
(ex : <> </> ||' <Fragment></Fragment>' )

8. 컴포넌트를 통해 data를 전달하는 경우에는 props를 사용하여 처리한다.
props는 일종의 속성을 칭하게 되며, 컴포넌트에서 속성명의 내용이 전달되는 인자(argument)의 객체명으로 전달이 된다.
...
`return` 이전의 위치에서는 변수, 함수....기눙의 형태를 배치하게 된다.
